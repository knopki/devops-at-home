...
if __name__ == "__main__":
    ...
    all_pkgs = load_doom_packages(root_path)
    ...
    bad_pkgs = filter(
        lambda x: not (x.fetcher and x.repo and x.rev and x.sha256), pkgs_with_hashes
    )
    ...
    good_pkgs = sorted(
        filter(lambda x: x.name not in bad_pkgs_names, pkgs_with_hashes),
        key=lambda x: x.name,
    )
