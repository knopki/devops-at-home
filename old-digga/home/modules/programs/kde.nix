...
with lib; let
  ...
  formatKConfigValue = v: let
    formatValue = generators.mkValueStringDefault {};
  in
    if v == null
    then ""
    else if isList v
    then concatMapStringsSep "," formatValue v
    else formatValue v;
  ...
  flattenKdeAttrs = let
    recurse = path: value:
      if isAttrs value
      then lib.mapAttrsToList (n: v: recurse ([n] ++ path) v) value
  ...
  lookAndFeelCmd =
    if (isString (cfg.settings.kdeglobals.KDE.LookAndFeelPackage or null))
    then ''
