...
let
  ...
  formatCmd =
    ...
    concatStringsSep " " (
      filter (v: v != "") [
        ...
        (optionalString (typeOf value != "null") (escapeShellArg (formatKConfigValue value)))
      ]
    );
  ...
  cmdList =
    ...
    ++ flatten (
      mapAttrsToList (
        filename: settings: map (formatCmd filename) (flattenKdeAttrs settings)
      ) cfg.settings
    )
    ++ [ reloadCmd ];
in
{
  ...
  config = mkIf cfg.enable {
    home.activation.kdeSettings = hm.dag.entryAfter [ "writeBoundary" ] (concatStringsSep "\n" cmdList);
  };
}
