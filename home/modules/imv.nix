# TODO: remove module after HM >= 23.11
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [ (hm.assertions.assertPlatform "programs.imv" pkgs platforms.linux) ];

    home.packages = [ cfg.package ];

    xdg.configFile = mkIf (cfg.settings != { }) { "imv/config".text = toConfig cfg.settings; };
  };
}
