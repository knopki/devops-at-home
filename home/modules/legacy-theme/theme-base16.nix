...
with lib; let
  inherit (self.lib.hex) toHex2 fromHex;
  cfg = config.theme;
  ...
  fromYamlFile = yamlFile: let
    json =
      ...
      '';
    bases =
      filterAttrs (n: _: hasPrefix "base" n)
  ...
  themeFile =
    if (cfg.preset != null && hasAttr cfg.preset presetFileMap)
    then presetFileMap."${cfg.preset}"
    else "${pkgs.base16-default-schemes}/default-dark.yaml";
in {
  options.theme.base16 = {
    ...
    kind = mkOption {
      ...
      example = "dark";
      default = let
        inherit (cfg.base16.colors.base00.dec) r g b;
      in
        if r + g + b >= 382
        then "light"
        else "dark";
