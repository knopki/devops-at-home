...
in
{
  ...
  config = mkIf (cfg.enable && cfg.components.gtk.enable) (mkMerge [
    ...
    (mkIf (!elem cfg.preset [ "dracula" ]) {
      ...
      gtk.theme = mkDefault materiaTheme;
      gtk.iconTheme = mkDefault papirusIcons;
    })

    (mkIf (cfg.preset == "dracula") {
      dconf.settings = {
        "org/gnome/desktop/wm/preferences".theme = mkDefault draculaTheme.name;
      };
      gtk.theme = mkDefault draculaTheme;
      gtk.iconTheme = mkDefault draculaIcons;
    })
  ]);
}
